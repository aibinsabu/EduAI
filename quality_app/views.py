...
def create_exam(request):
    ...
    if request.method == 'POST':
        ...
        num_mcq = int(request.POST.get('num_mcq', 0)) if exam_type == 'Mixed' else None
        num_essay = int(request.POST.get('num_essay', 0)) if exam_type == 'Mixed' else None
        ...
        if request.POST.get('creation_method') == 'AI' or True: # Force AI for demo if needed, or check a flag
             ...
             questions = generate_questions(context_text, max_questions=num_questions, 
                                          ...
                                          num_mcq=num_mcq, num_essay=num_essay)
        ...
        if generated_questions_data:
            for q_data in generated_questions_data:
                try:
                    ...
                    q_type = q_data.get('type', 'SAQ')
                    q_opts = json.dumps(q_data.get('options')) if q_data.get('options') else None
    ...
    for q in Question.objects.filter(exam=exam):
        ...
        if q.question_type == 'MCQ':
            ...
            grade = {
                ...
                "feedback": "Correct!" if is_correct else f"Incorrect. Correct answer: {q.answer}"
            }
        else:
            # AI Similarity for Essay/SAQ
